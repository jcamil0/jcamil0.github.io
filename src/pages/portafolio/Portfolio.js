...
const Portfolio = () => {
    ...
    return (
        <Grid container className="section pd_45 pt_45">
            ...
            <Grid item xs={12}>
                <Tabs
                    ...
                    {[...new Set(Personaldata.portfolio.map(item => item.tag))].map(tag => (
                        <Tab label={tag} value={tag} className={Tabvalue === tag ? 'custonTab_item activate' : 'custonTab_item'} key={tag}/>
                    ))}
                </Tabs>
            </Grid>
                                ...
                                <Grid item xs={12} sm={6} md={4} >
                                    <Grow in timeout={1000}>
                                        <Card className='customCard' onClick={() => setProyectDialog(proyect)}>
                                            <CardActionArea>
                                                <CardMedia key={proyect.img} className='customCard_image' style={{ height: "140px" }} component="img" image={proyect.img} />
                                                <CardContent>
                                                    <Typography  key={proyect.title}className='customCard_title'>{proyect.title}</Typography>
                                                    <Typography key={proyect.caption} className='customCard_caption' varinat='body2' >{proyect.caption}</Typography>
                                                </CardContent>
                                            </CardActionArea>
                                        </Card>
                                    </Grow>
            ...
            {/* modal screen */}
            <Dialog open={proyectDialog} onClose={() => setProyectDialog(false)} className='proyectDialog'>
                ...
                <DialogContent>
                    <Typography className='proyectDialog_description'>{proyectDialog.description}</Typography>
                </DialogContent>
                <DialogActions className='proyectDialog_actions'>
                    {proyectDialog?.links?.map(link => (
                        <a href={link.link} target='_blank' className='proyectDialog_icon'>{link.icon}</a>
                    ))}
                </DialogActions>
            </Dialog>
        </Grid>
